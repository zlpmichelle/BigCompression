...
package org.apache.hadoop.io.orc;
...
import org.apache.hadoop.io.orc.CompressionKind;
import org.apache.hadoop.io.orc.OrcFile;
import org.apache.hadoop.io.orc.Writer;
 ...
 * unit of work when reading. Each stripe is buffered in memory until the memory
 * reaches the stripe size and then it is written out broken down by columns.
 * Each column is written by a TreeWriter that is specific to that type of
 * column. TreeWriters may have children TreeWriters that handle the sub-types.
 * Each of the TreeWriters writes the column's data as a set of streams.
 */
class WriterImpl implements Writer, MemoryManager.Callback {
  ...
  private final OrcProto.RowIndex.Builder rowIndex = OrcProto.RowIndex
      .newBuilder();
  private final boolean buildIndex;
  private final MemoryManager memoryManager;

  WriterImpl(FileSystem fs, Path path, ObjectInspector inspector,
      long stripeSize, CompressionKind compress, int bufferSize,
   ...
   * The TreeWriters write to the outStream and the codec compresses the data as
   * buffers fill up and stores them in the output list. When the stripe is
   * being written, the whole stream is written to the file.
   */
  private class BufferedStream implements OutStream.OutputReceiver {
    ...
    BufferedStream(String name, int bufferSize, CompressionCodec codec)
        throws IOException {
  ...
  /**
   * An output receiver that writes the ByteBuffers to the output stream as they
   * are received.
   */
  ...
  /**
   * The parent class of all of the writers for each column. Each column is
   * written by an instance of this class. The compound types (struct, list,
   * map, and union) have children tree writers that write the children types.
   */
  private abstract static class TreeWriter {
     ...
     * 
     * @param columnId
     *          the column id of the column to write
     * @param inspector
     *          the object inspector to use
     * @param streamFactory
     *          limited access to the Writer's data.
     * @param nullable
     *          can the value be null?
     * @throws IOException
     */
    TreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory streamFactory, boolean nullable) throws IOException {
      this.id = columnId;
     ...
     * 
     * @param builder
     *          the stripe footer that contains the information about the layout
     *          of the stripe. The TreeWriter is required to update the footer
     *          with its information.
     * @param requiredIndexEntries
     *          the number of index entries that are required. this is to check
     *          to make sure the row index is well formed.
     * @throws IOException
     */
    ...
    /**
     * Create a row index entry with the previous location and the current index
     * statistics. Also merges the index statistics into the file statistics
     * before they are cleared. Finally, it records the start of the next index
     * and ensures all of the children columns also create an entry.
     * 
     * @throws IOException
     */
     ...
     * 
     * @param recorder
     *          where should the locations be recorded
     * @throws IOException
     */
  ...
  private static class BooleanTreeWriter extends TreeWriter {
    ...
    BooleanTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
  ...
  private static class ByteTreeWriter extends TreeWriter {
    ...
    ByteTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
  ...
  private static class IntegerTreeWriter extends TreeWriter {
    ...
    IntegerTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      PositionedOutputStream out = writer.createStream(id,
          OrcProto.Stream.Kind.DATA);
  ...
  private static class FloatTreeWriter extends TreeWriter {
    ...
    FloatTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
  ...
  private static class DoubleTreeWriter extends TreeWriter {
    ...
    DoubleTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      this.stream = writer.createStream(id, OrcProto.Stream.Kind.DATA);
      recordPosition(rowIndexPosition);
  ...
  private static class StringTreeWriter extends TreeWriter {
    ...
    StringTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      ...
      for (int i = 0; i <= length; ++i) {
        ...
        if (buildIndex) {
          while (i == rowIndexValueCount.get(rowIndexEntry)
            ...
            OrcProto.RowIndexEntry.Builder base = savedRowIndex.get(
                rowIndexEntry++).toBuilder();
  ...
  private static class BinaryTreeWriter extends TreeWriter {
    ...
    BinaryTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      this.stream = writer.createStream(id, OrcProto.Stream.Kind.DATA);
    ...
    void write(Object obj) throws IOException {
      super.write(obj);
      if (obj != null) {
        BytesWritable val = ((BinaryObjectInspector) inspector)
  ...
  static final long BASE_TIMESTAMP = Timestamp.valueOf("2015-01-01 00:00:00")
  ...
  private static class TimestampTreeWriter extends TreeWriter {
    ...
    TimestampTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
    ...
    void write(Object obj) throws IOException {
      super.write(obj);
      if (obj != null) {
        Timestamp val = ((TimestampObjectInspector) inspector)
            .getPrimitiveJavaObject(obj);
  ...
  private static class DecimalTreeWriter extends TreeWriter {
    ...
    DecimalTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
    ...
    void write(Object obj) throws IOException {
      ...
      if (obj != null) {
        HiveDecimal decimal = ((HiveDecimalObjectInspector) inspector)
            .getPrimitiveJavaObject(obj);
        SerializationUtils
            .writeBigInteger(valueStream, decimal.unscaledValue());
  ...
  private static class StructTreeWriter extends TreeWriter {
    ...
    StructTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      ...
      for (int i = 0; i < childrenWriters.length; ++i) {
        childrenWriters[i] = createTreeWriter(fields.get(i)
            .getFieldObjectInspector(), writer, true);
      }
      recordPosition(rowIndexPosition);
    }
  ...
  private static class ListTreeWriter extends TreeWriter {
    ...
    ListTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      ListObjectInspector listObjectInspector = (ListObjectInspector) inspector;
      childrenWriters = new TreeWriter[1];
      childrenWriters[0] = createTreeWriter(
          listObjectInspector.getListElementObjectInspector(), writer, true);
  ...
  private static class MapTreeWriter extends TreeWriter {
    ...
    MapTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      MapObjectInspector insp = (MapObjectInspector) inspector;
      childrenWriters = new TreeWriter[2];
      childrenWriters[0] = createTreeWriter(insp.getMapKeyObjectInspector(),
          writer, true);
      childrenWriters[1] = createTreeWriter(insp.getMapValueObjectInspector(),
          writer, true);
      lengths = new RunLengthIntegerWriter(writer.createStream(columnId,
          OrcProto.Stream.Kind.LENGTH), false);
      recordPosition(rowIndexPosition);
  ...
  private static class UnionTreeWriter extends TreeWriter {
    ...
    UnionTreeWriter(int columnId, ObjectInspector inspector,
        StreamFactory writer, boolean nullable) throws IOException {
      super(columnId, inspector, writer, nullable);
      ...
      tags = new RunLengthByteWriter(writer.createStream(columnId,
          OrcProto.Stream.Kind.DATA));
      recordPosition(rowIndexPosition);
    }
  ...
  private static TreeWriter createTreeWriter(ObjectInspector inspector,
    ...
    case PRIMITIVE:
      ...
      case BYTE:
        return new ByteTreeWriter(streamFactory.getNextColumnId(), inspector,
      ...
      case LONG:
        return new IntegerTreeWriter(streamFactory.getNextColumnId(),
            inspector, streamFactory, nullable);
      case FLOAT:
        return new FloatTreeWriter(streamFactory.getNextColumnId(), inspector,
            streamFactory, nullable);
      case DOUBLE:
        return new DoubleTreeWriter(streamFactory.getNextColumnId(), inspector,
            streamFactory, nullable);
      case STRING:
        return new StringTreeWriter(streamFactory.getNextColumnId(), inspector,
            streamFactory, nullable);
      case BINARY:
        return new BinaryTreeWriter(streamFactory.getNextColumnId(), inspector,
            streamFactory, nullable);
      case TIMESTAMP:
  ...
  private void ensureWriter() throws IOException {
    if (rawWriter == null) {
      ...
      writer = new OutStream("metadata", bufferSize, codec, new DirectStream(
          rawWriter));
  ...
  private void flushStripe() throws IOException {
    ...
    if (rowsInStripe != 0) {
      int requiredIndexEntries = rowIndexStride == 0 ? 0 : (int) ((rowsInStripe
          + rowIndexStride - 1) / rowIndexStride);
      OrcProto.StripeFooter.Builder builder = OrcProto.StripeFooter
          .newBuilder();
      ...
      for (Map.Entry<StreamName, BufferedStream> pair : streams.entrySet()) {
        ...
        builder.addStreams(OrcProto.Stream.newBuilder()
            .setColumn(name.getColumn()).setKind(name.getKind())
      ...
      OrcProto.StripeInformation dirEntry = OrcProto.StripeInformation
          .newBuilder().setOffset(start).setIndexLength(indexEnd - start)
          .setDataLength(section - indexEnd).setNumberOfRows(rowsInStripe)
