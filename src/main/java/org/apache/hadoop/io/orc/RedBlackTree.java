...
import org.apache.hadoop.io.orc.DynamicIntArray;

/**
 * A memory efficient red-black tree that does not allocate any objects per an
 * element. This class is abstract and assumes that the child class handles the
 * key and comparisons with the key.
 */
abstract class RedBlackTree {
   ...
   * 
   * @return 0 if the values are the same, -1 if the new value is smaller and 1
  ...
  /**
   * Set the left field of the given position. Note that we are storing the node
   * color in the low bit of the left pointer.
   */
   ...
   * invariants: 1. If a node is red, both of its children are black. 2. Each
   * child of a node has the same black height (the number of black nodes
   * between it and the leaves of the tree).
   * 
   ...
   * 
   * @param node
   *          The node that we are fixing right now.
   * @param fromLeft
