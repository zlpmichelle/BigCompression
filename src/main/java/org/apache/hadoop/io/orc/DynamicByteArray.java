...
final class DynamicByteArray {
   ...
   * @param valueLength
   *          the number of bytes to copy from value
   * @return
   */
  ...
  public void readAll(InputStream in) throws IOException {
    ...
    grow(currentChunk);
    int currentLength = in.read(data[currentChunk], currentOffset, chunkSize
        - currentOffset);
    while (currentLength > 0) {
      ...
      currentLength = in.read(data[currentChunk], currentOffset, chunkSize
          - currentOffset);
    }
  }
   ...
   * 
   * @param other
   *          source of the other bytes
   * @param otherOffset
   *          start offset in the other array
   * @param otherLength
   *          number of bytes in the other array
   * @param ourOffset
   *          the offset in our array
   * @param ourLength
   ...
   *          the value to set
   * @param offset
   *          the start of the bytes to copy
   * @param length
