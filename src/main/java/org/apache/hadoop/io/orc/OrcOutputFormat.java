...
public class OrcOutputFormat extends
    FileOutputFormat<NullWritable, OrcSerdeRow> implements
    HiveOutputFormat<NullWritable, OrcSerdeRow> {

  private static class OrcRecordWriter implements
      RecordWriter<NullWritable, OrcSerdeRow>, FileSinkOperator.RecordWriter {
    ...
    OrcRecordWriter(FileSystem fs, Path path, Configuration conf,
        String stripeSize, String compress, String compressionSize,
    ...
    @Override
    public void write(NullWritable nullWritable, OrcSerdeRow row)
        throws IOException {
      if (writer == null) {
  ...
  public RecordWriter<NullWritable, OrcSerdeRow> getRecordWriter(
      FileSystem fileSystem, JobConf conf, String name, Progressable reporter)
      throws IOException {
  ...
  public FileSinkOperator.RecordWriter getHiveRecordWriter(JobConf conf,
      ...
      Properties tableProperties, Progressable reporter) throws IOException {
    ...
    String compressionSize = tableProperties.getProperty(
        OrcFile.COMPRESSION_BLOCK_SIZE, OrcFile.DEFAULT_COMPRESSION_BLOCK_SIZE);
    String rowIndexStride = tableProperties.getProperty(
        OrcFile.ROW_INDEX_STRIDE, OrcFile.DEFAULT_ROW_INDEX_STRIDE);
