...
import org.apache.hadoop.io.orc.PositionRecorder;
import org.apache.hadoop.io.orc.PositionedOutputStream;
import org.apache.hadoop.io.orc.SerializationUtils;

/**
 * A streamFactory that writes a sequence of integers. A control byte is written
 * before each run with positive values 0 to 127 meaning 3 to 130 repetitions,
 * each repetition is offset by a delta. If the control byte is -1 to -128, 1 to
 * 128 literal vint values follow.
 */
class RunLengthIntegerWriter {
  ...
  RunLengthIntegerWriter(PositionedOutputStream output, boolean signed) {
    this.output = output;
    this.signed = signed;
  }
