...
import org.apache.hadoop.io.orc.InStream;
import org.apache.hadoop.io.orc.PositionProvider;
import org.apache.hadoop.io.orc.RunLengthByteWriter;

/**
 * A reader that reads a sequence of bytes. A control byte is read before each
 * run with positive values 0 to 127 meaning 3 to 130 repetitions. If the byte
 * is -1 to -128, 1 to 128 literal byte values follow.
 */
class RunLengthByteReader {
  private final InStream input;
  private final byte[] literals = new byte[RunLengthByteWriter.MAX_LITERAL_SIZE];
