...
class OutStream extends PositionedOutputStream {
  ...
  /**
   * Write the length of the compressed bytes. Life is much easier if the header
   * is constant length, so just use 3 bytes. Considering most of the codecs
   * want between 32k (snappy) and 256k (lzo, zlib), 3 bytes should be plenty.
   ...
   *          is it uncompressed
   */
  private static void writeHeader(ByteBuffer buffer, int position, int val,
      boolean original) {
  ...
  private ByteBuffer getNewOutputBuffer() throws IOException {
    return ByteBuffer.allocate(bufferSize + (codec == null ? 0 : HEADER_SIZE));
  }
