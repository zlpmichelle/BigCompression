...
import org.apache.hadoop.io.orc.RecordReader;
import org.apache.hadoop.io.orc.StripeInformation;
...
class RecordReaderImpl implements RecordReader {
  ...
  RecordReaderImpl(Iterable<StripeInformation> stripes, FileSystem fileSystem,
      Path path, long offset, long length, List<OrcProto.Type> types,
      CompressionCodec codec, int bufferSize, boolean[] included,
      long strideRate) throws IOException {
  ...
  private abstract static class TreeReader {
    ...
    void startStripe(Map<StreamName, InStream> streams,
        List<OrcProto.ColumnEncoding> encoding) throws IOException {
     ...
     * 
     * @param index
     *          the indexes loaded from the file
     * @throws IOException
     */
  ...
  private static class BooleanTreeReader extends TreeReader {
    ...
    @Override
    void startStripe(Map<StreamName, InStream> streams,
        List<OrcProto.ColumnEncoding> encodings) throws IOException {
  ...
  private static class DecimalTreeReader extends TreeReader {
    ...
    void startStripe(Map<StreamName, InStream> streams,
      ...
      valueStream = streams.get(new StreamName(columnId,
          OrcProto.Stream.Kind.DATA));
      scaleStream = new RunLengthIntegerReader(streams.get(new StreamName(
          columnId, OrcProto.Stream.Kind.SECONDARY)), true);
    }
  ...
  private static class StructTreeReader extends TreeReader {
    ...
    StructTreeReader(int columnId, List<OrcProto.Type> types, boolean[] included)
        throws IOException {
      super(columnId);
  ...
  private static class UnionTreeReader extends TreeReader {
    ...
    UnionTreeReader(int columnId, List<OrcProto.Type> types, boolean[] included)
        throws IOException {
      super(columnId);
      OrcProto.Type type = types.get(columnId);
    ...
    Object next(Object previous) throws IOException {
      ...
      if (valuePresent) {
        ...
        result.set(tag,
            fields[tag].next(tag == result.getTag() ? previousVal : null));
      }
      return result;
    }
  ...
  private static class ListTreeReader extends TreeReader {
    ...
    ListTreeReader(int columnId, List<OrcProto.Type> types, boolean[] included)
        throws IOException {
      super(columnId);
      OrcProto.Type type = types.get(columnId);
    ...
    Object next(Object previous) throws IOException {
      ...
      if (valuePresent) {
        ...
        for (int i = 0; i < length; i++) {
          result.set(i,
              elementReader.next(i < prevLength ? result.get(i) : null));
        }
  ...
  private static class MapTreeReader extends TreeReader {
    ...
    MapTreeReader(int columnId, List<OrcProto.Type> types, boolean[] included)
        throws IOException {
      super(columnId);
      OrcProto.Type type = types.get(columnId);
  ...
  private static TreeReader createTreeReader(int columnId,
      List<OrcProto.Type> types, boolean[] included) throws IOException {
    OrcProto.Type type = types.get(columnId);
    switch (type.getKind()) {
    case BOOLEAN:
      ...
      for (OrcProto.Stream section : stripeFooter.getStreamsList()) {
        if (StreamName.getArea(section.getKind()) == StreamName.Area.DATA) {
          ...
          StreamName name = new StreamName(section.getColumn(),
              section.getKind());
          streams.put(name, InStream.create(name.toString(), sectionBuffer,
              codec, bufferSize));
      ...
      while (currentSection < streamList.size()) {
        ...
        if (bytes != 0) {
          ...
          while (currentSection < excluded) {
            ...
            StreamName name = new StreamName(section.getColumn(),
                section.getKind());
            this.streams.put(name, InStream.create(name.toString(),
                ByteBuffer.wrap(buffer, bytes, (int) section.getLength()),
                codec, bufferSize));
            currentSection += 1;
  ...
  /**
   * Return the fraction of rows that have been read from the selected. section
   * of the file
   * 
