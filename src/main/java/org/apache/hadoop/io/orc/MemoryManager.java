...
class MemoryManager {
  ...
  /**
   * Update the currentScale based on the current allocation and pool size. This
   * also updates the notificationTrigger.
   * 
   * @param isAllocate
  ...
  private void updateScale(boolean isAllocate) throws IOException {
    ...
    if (!isAllocate) {
      ...
      notificationTrigger = Math.min(notificationTrigger,
