...
public interface Reader {
   ...
   * 
   * @return the number of rows per an entry in the row index or 0 if there is
   *         no row index.
   */
   ...
   * offset up to the stripe that starts at offset + length. This is intended to
   * work with MapReduce's FileInputFormat where divisions are picked blindly,
   * but they must cover all of the rows.
   * 
   * @param offset
   *          a byte offset in the file
   * @param length
   *          a number of bytes in the file
   * @param include
   *          true for each column that should be included
