...
import org.apache.hadoop.io.orc.CompressionCodec;
...
class SnappyCodec implements CompressionCodec {
  ...
  public boolean compress(ByteBuffer in, ByteBuffer out, ByteBuffer overflow)
    ...
    int outBytes = Snappy.compress(in.array(),
        in.arrayOffset() + in.position(), inBytes, compressed, 0);
    if (outBytes < inBytes) {
      int remaining = out.remaining();
      if (remaining >= outBytes) {
        System.arraycopy(compressed, 0, out.array(),
            out.arrayOffset() + out.position(), outBytes);
        out.position(out.position() + outBytes);
      } else {
        System.arraycopy(compressed, 0, out.array(),
            out.arrayOffset() + out.position(), remaining);
  ...
  public void decompress(ByteBuffer in, ByteBuffer out) throws IOException {
    ...
    int uncompressLen = Snappy.uncompress(in.array(), in.arrayOffset()
        + inOffset, in.limit() - inOffset, out.array(),
